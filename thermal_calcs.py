...
def calculate_wall_temperature(T_initial, delta_t,T_ad,t_wall,pc,oxname,fuelname,of,eps,mach_exit,burn_time,D_c,c_star,D_t,D_e):
    """
    ...
    for T_w in T_array:
        ...
        for location in locations:
            ...
            for t in time_array:
                h_g = calculate_heat_transfer_coefficient(T_w,T_ad,pc,c_star=c_star,A_t=A_t_m2,A=slice_area,D_t=D_t,r_c=1,M=M,oxname=oxname,fuelname=fuelname,of=of,eps=eps,location=location) #fix all set values 
                q_in = h_g*(T_aw-T_1)
                q_cond = G*(T_1-T_2)
