...
def ablative_rate(T_initial, delta_t,T_ad,T_0g,t_wall,pc,oxname,fuelname,of,eps,mach_exit,burn_time,D_c,c_star,D_t,D_e,emissivity = 0.3, boltzmann = 5.67e-8, T_amb = 298, abl_t0 = 5):
    ...
    N_ab = 100
    ab_step = abl_t0/N_ab
    X_dict = {i: {"level": round(abl_t0 - i*ab_step, 2), 'state':'v', 'temp':T_initial} for i in range(0, N_ab+1)}
    print(X_dict)
    ...
    for location in locations:
        ...
        for t in time_array:
            ...
            T_1 = T_1 + (delta_t / C) * (q_in - q_cond-q_rad)
            T_2 = T_2 + (delta_t / C) * (q_cond - q_rad2)
